...
def process_photodiode_data(dir_path: str, file_number: int):
    ramp_path = os.path.join(dir_path, f"C2_RL_{file_number:05d}.txt")

    # Load and process ramp data
    Ramp = np.loadtxt(ramp_path, skiprows=5)[:, 1]
    Ramp_smooth = scipy.signal.savgol_filter(Ramp, 20, 1)
    Derivative_Ramp = np.gradient(Ramp_smooth)
    Derivative_Ramp = Derivative_Ramp / max(Derivative_Ramp)
    ...
    if len(Derivative_Peaks) != 4:
        print("Check Ramp — expected 4 derivative peaks.")
        return
    ...
    photodiode_path = os.path.join(dir_path, f"F1_RL_{file_number:05d}.txt")
    ...
    delta_min = ((11.32 * min(Ramp) + 131.79) + AOM_calibration.LockingFrequencyProbe + AOM_calibration.SPassProbe) / 6.06
    delta_max = ((11.32 * max(Ramp) + 131.79) + AOM_calibration.LockingFrequencyProbe + AOM_calibration.SPassProbe) / 6.06
    Delta_list_PD = np.linspace(delta_min, delta_max, size)

    t_pd = (With_atoms - background) / (Without_atoms - background)

    Points_to_remove = 300
    if Points_to_remove != 0:
        Delta_list_PD = Delta_list_PD[Points_to_remove:-Points_to_remove]
        t_pd = t_pd[Points_to_remove:-Points_to_remove]
        With_atoms = With_atoms[Points_to_remove:-Points_to_remove]
        Without_atoms = Without_atoms[Points_to_remove:-Points_to_remove]

    param_pd, _ = curve_fit(BL2, Delta_list_PD, t_pd, p0=[10, 0.1, 0], maxfev=int(1e6))

    print('Estimated OD of cloud is = {:.2f}'.format(param_pd[0]))

    fig, ax = plt.subplots()
    ax.plot(Delta_list_PD, t_pd, '.', alpha=0.3, label='Photodiode')
    ax.plot(Delta_list_PD, BL2(Delta_list_PD, *param_pd), '-', c='b', label='Fit - OD = {:.2f}'.format(param_pd[0]))
    ax.set_xlabel('Detuning (Gamma)')
    ax.set_ylabel('Transmission')
    ax.legend()
    ax.grid()
...
def main():
    parser = argparse.ArgumentParser(description="Process photodiode data for cold atom absorption.")
    parser.add_argument("--year", required=True, help="Year folder (e.g. 2025)")
    parser.add_argument("--month", required=True, help="Month folder (e.g. 06)")
    parser.add_argument("--date", required=True, help="Date folder (e.g. 07)")
    parser.add_argument("--file", required=True, type=int, help="File number to process (e.g. 11)")
    args = parser.parse_args()

    dir_path = os.path.join("data", args.year, args.month, args.date)
    if not os.path.isdir(dir_path):
        raise FileNotFoundError(f"Directory does not exist: {dir_path}")

    process_photodiode_data(dir_path, args.file)
